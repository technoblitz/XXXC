...
// Copyright (c) 2013 The XXXBTC developers
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    std::string developerWallet = "fgfff3DLyymMkZajpksc5HLtNw5Ggrgrgr";
    ...
    {
		 //txNew.vout[0].nValue = 0.99 * ((CScript() << pindexPrev->nHeight+1) + COINBASE_FLAGS);
    //txNew.vout[1].nValue = 0.01 * ((CScript() << pindexPrev->nHeight+1) + COINBASE_FLAGS;
	
        // Height first in coinbase required for block.version=2
        txNew.vin[0].scriptSig = 0.99 * ((CScript() << pindexPrev->nHeight+1) + COINBASE_FLAGS);
	 txNew.vin[1].scriptSig = 0.01 * ((CScript() << pindexPrev->nHeight+1) + COINBASE_FLAGS);
